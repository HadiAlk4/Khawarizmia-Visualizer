...
const PrefixCalc = () => {
        ...
        if (currentToken !== null) {
            if (/\d+/.test(currentToken)) {
                ...
                while (
                    ...
                    Precedence[currentStack[currentStack.length - 1]] > (Precedence[currentToken] || 0) 
                ) {
                    const poppedOP = currentStack.pop() as string;
                    currentOutput.push(poppedOP);
                }
    ...
    const handleStepBack = () => {
        ...
        const lastSnapshot = newHistory.pop() as HistorySnapshot;

        setInputTokens(lastSnapshot.input);
        setCurrentToken(lastSnapshot.current);
        setStack(lastSnapshot.stack);
        setOutput(lastSnapshot.output);
    ...
    return (
        <div className="relative min-h-screen bg-slate-50 p-8 flex flex-col items-center gap-8 font-kufi bg-[url('/overlapping-circles.svg')]">
            ...
            <div className="w-full max-w-4xl grid grid-cols-2 gap-12 mt-4">
                ...
                <div className="flex flex-col items-center gap-2">
                    <h2 className="text-xl font-bold bg-black text-white px-4 py-1"> Prefix Output </h2>
